...
$(function () {
  ...
  var features = [{ label: 'Block Facebook posts', index: 'fb' }, { label: 'Block Google Results', index: 'google' }, { label: 'Redirect from sites', index: 'redirect' }, { label: 'Link warnings', index: 'warnings' }];
  var manageEl = $('.manage');
  ...
  $('body').on('click', '.switch-input', function () {
    ...
    featureSettings[feat] = $(this).prop('checked');
    setData({ features: featureSettings });
  });

  // delete exisiting blocked sites
  $('body').on('click', '.manage li a', function () {
    var that = this;
  ...
  $('.add-url-btn').on('click', function (e) {
    ...
    chrome.storage.sync.get(settings, function (res) {
      ...
      if (url !== '' && ValidURL(url)) {
        ...
        // clear the form
        $('.add-url').val('');
        $('.add-url-success').css('display', 'block');

        setTimeout(function () {
          $('.add-url-success').css('display', 'none');
        }, 3000);
      } else {
        $('.add-url-error').css('display', 'block');
      }
    });
  });

  // clear error
  $('.add-url').on('keyup', function () {
    if ($('.add-url').val() !== '') {
      $('.add-url-error').css('display', 'none');
    }
  });
  ...
  function setupFeats() {
    chrome.storage.sync.get(settings, function (res) {
      var featuresEnabled = res.features;

      $.each(features, function (k, v) {
        var checked = '';

        if (featuresEnabled[v.index]) {
          checked = 'checked=checked';
        }

        var tmp = "<li>\n          <span class=\"label\">" + v.label + "</span>\n          <label class=\"switch\">\n            <input type=\"checkbox\" class=\"switch-input\" value=\"1\" " + checked + " data-feature=\"" + v.index + "\">\n            <span class=\"switch-label\" data-on=\"On\" data-off=\"Off\"></span>\n            <span class=\"switch-handle\"></span>\n          </label>\n        </li>";
        featuresEl.append(tmp);
      });
    });
  }

  function managableUrls() {
    manageEl.html('');
    chrome.storage.sync.get(settings, function (res) {
      var urls = res.urls;
      ...
      $.each(urls, function (k, v) {
        ...
        var tmp = "<li><a href=\"#\" data-index=\"" + k + "\">Delete</a> " + v + "</li>";
        manageEl.append(tmp);
      });
    });
  }
  ...
  function setData(obj) {
    // chrome.storage.sync.get({features}, function(urls) {
